...
const AdminDashboard = () => {
  ...
  const handleStatChange = (index, field, value) => {
    ...
    setProfileData({ ...profileData, stats: newStats });
  };

  const addStat = () => {
    setProfileData({
      ...profileData,
      stats: [...profileData.stats, { label: 'New Stat', value: 50, color: 'bg-blue-500' }]
    });
  };

  const removeStat = (index) => {
    const newStats = profileData.stats.filter((_, i) => i !== index);
    setProfileData({ ...profileData, stats: newStats });
  };
            ...
            <form onSubmit={handleProfileUpdate} className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="text-yellow-500 font-mono text-sm">ATTRIBUTES (STATS)</label>
                  <button type="button" onClick={addStat} className="text-green-500 text-sm hover:text-green-400 font-bold">+ ADD ATTRIBUTE</button>
                </div>

                <div className="space-y-4">
                  {profileData.stats.map((stat, index) => (
                    <div key={index} className="flex gap-4 items-center bg-gray-800 p-4 rounded-lg">
                      <input
                        type="text"
                        value={stat.label}
                      ...
                      <input
                        ...
                        className="w-20 bg-gray-700 text-white px-3 py-2 rounded border border-gray-600"
                      />
                      <select
                        value={stat.color}
                        onChange={(e) => handleStatChange(index, 'color', e.target.value)}
                        className="w-32 bg-gray-700 text-white px-3 py-2 rounded border border-gray-600"
                      >
                      ...
                      <button type="button" onClick={() => removeStat(index)} className="text-red-500 hover:text-red-400">
                        <FiTrash2 />
                      </button>
                    </div>
                  ))}
